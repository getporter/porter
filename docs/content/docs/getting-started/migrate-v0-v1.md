...
4. The `invocationImage` field has been deprecated and is no longer available.
   Porter now generates the bundle image name using a hash generated from the bundle's metadata using the following format: BUNDLE_REPOSITORY:porter-HASH.
   We also no longer push the bundle image to a separate repository, so if you used to rely on the BUNDLE_NAME-installer naming convention, that is not available going forward.
