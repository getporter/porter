...
resource IDs created during install that need to be referenced during upgrade or uninstall.
...
on when to use parameter sources versus state files, with working examples.
...
Outputs can be used with [parameter sources](#parameter-sources) to persist data between bundle actions. See
[Persisting Data Between Bundle Actions](/docs/development/authoring-a-bundle/persisting-data/) for guidance on using
outputs with parameter sources to remember data across install, upgrade, and uninstall.
...
**Note:** State files cannot be accessed via templates or environment variables. If you need to access
values in your bundle steps, use [parameter sources](#parameter-sources) instead. See
[Persisting Data Between Bundle Actions](/docs/development/authoring-a-bundle/persisting-data/) for guidance
on choosing between state files and parameter sources.
