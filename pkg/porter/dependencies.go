...
type queuedDependency struct {
	...
	// Output files generated by the dependency stored in PORTER_HOME
	// map from the file path to the file contents
...
func (e *dependencyExecutioner) ApplyDependencyMappings(args *cnabprovider.ActionArguments) {
	...
	for _, dep := range e.deps {
		...
		for output, value := range dep.outputs {
			...
			args.Files[target] = string(value)
		}
	}
}
...
func (e *dependencyExecutioner) prepareDependency(dep *queuedDependency) error {
	...
	// Remember which outputs we want to cache
	outputsDir, err := e.Config.GetOutputsDir()
	if err != nil {
		return errors.Wrap(err, "unable to get outputs directory")
	}

	dep.outputs = make(map[string][]byte, len(depBun.Outputs))
	for outputName := range depBun.Outputs {
		depClaim := fmt.Sprintf("%s-%s", e.parentOpts.Name, dep.Alias)
		depOutputsDir := filepath.Join(outputsDir, depClaim)
		outputPath := filepath.Join(depOutputsDir, outputName)
		dep.outputs[outputPath] = nil // We will cache the value of the output after the dep is executed
	}
...
func (e *dependencyExecutioner) executeDependency(dep *queuedDependency, parentArgs cnabprovider.ActionArguments) error {
	...
	for output := range dep.outputs {
		dep.outputs[output], err = e.FileSystem.ReadFile(output)
		if err != nil {
			return errors.Wrapf(err, "error reading output file %s", output)
		}
	}
