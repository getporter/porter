...
func TestExtendedBundle_ResolveVersion(t *testing.T) {
	...
	testcases := []struct {
		...
		{name: "range with exact strategy and pinned tag uses tag",
			dep:         depsv1ext.Dependency{Bundle: "mysql:5.7", Version: &depsv1ext.DependencyVersion{Ranges: []string{">=1.0 <1.5"}}},
			wantVersion: "5.7"},
...
func TestExtendedBundle_ResolveVersionv2(t *testing.T) {
	...
	testcases := []struct {
		...
		{name: "constraint without strategy errors",
			...
			wantError: "version strategy is"},
		{name: "constraint with exact strategy and pinned tag uses tag",
			dep:         v2.Dependency{Bundle: "mysql:5.7", Version: ">=1.0 <1.3"},
			wantVersion: "5.7"},
