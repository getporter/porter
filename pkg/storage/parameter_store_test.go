...
func TestParameterStore_CRUD(t *testing.T) {
	for _, paramStore := range []*TestParameterSetProvider{
		NewTestParameterProvider(t),
		NewTestParameterProviderSQL(t),
	} {
		t.Run(paramStore.Name, func(t *testing.T) {
			...
			require.NoError(t, err)
			// set time zones to UTC to avoid equality issues
			pset.Status.Created = pset.Status.Created.UTC()
			pset.Status.Modified = pset.Status.Modified.UTC()
