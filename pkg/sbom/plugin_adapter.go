...
// PluginAdapter converts between the low-level plugins.SBOMGeneratorProtocol and
// the sbom.SBOMGenerator interface.
type PluginAdapter struct {
	plugin plugins.SBOMGeneratorProtocol
}

// NewPluginAdapter wraps the specified sbom generator plugin.
func NewPluginAdapter(plugin plugins.SBOMGeneratorProtocol) PluginAdapter {
	return PluginAdapter{plugin: plugin}
}

func (a PluginAdapter) Generate(
	ctx context.Context,
	bundleRef string,
	sbomPath string,
