...
// SBOMGenerator is a plugin-backed source of sbom generation. It resolves the appropriate
...
func (s *SBOMGenerator) Connect(ctx context.Context) error {
	...
	pluginType := NewSBOMGeneratorPluginConfig()
	...
	if !ok {
		...
		return span.Error(
			fmt.Errorf(
				"the interface (%T) exposed by the %s plugin was not plugins.SBOMGeneratorProtocol",
				conn.GetClient(),
				conn,
			),
		)
