...
func (c *PluginConnection) Start(ctx context.Context, pluginCfg io.Reader) error {
	...
	if err != nil {
		...
		if stderr := errbuf.String(); stderr != "" {
			err = fmt.Errorf("%s: %w", err.Error(), errors.New(stderr))
		}
		return span.Error(fmt.Errorf("could not connect to the %s plugin: %w", c.key, err))
	}

	err = c.setUpDebugger(ctx, c.client)
	if err != nil {
		c.Close(ctx)
		return span.Error(fmt.Errorf("could not set up debugger for plugin: %w", err))
	}
