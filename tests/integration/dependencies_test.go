...
func publishMySQLBundle(p *porter.TestPorter) {
	...
	p.TestConfig.TestContext.AddTestDirectory(filepath.Join(p.RepoDir, "build/testdata/bundles/mysql"), bunDir)
	pwd := p.Getwd()
	p.Chdir(bunDir)
	defer p.Chdir(pwd)
...
func installWordpressBundle(p *porter.TestPorter) (namespace string) {
	...
	p.CopyDirectory(filepath.Join(p.RepoDir, "build/testdata/bundles/wordpress"), ".", false)
